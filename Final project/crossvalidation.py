...
def crossval_Adaboost(n_fold, train_data, train_labels, nbound_range):
    n_nbound = len(nbound_range)
    micro_prec = np.zeros(n_nbound)
    macro_prec = np.zeros(n_nbound)
    macro_rec = np.zeros(n_nbound)
    subset_size = len(train_data)/n_fold
    for i in range(n_nbound):
        nbound = nbound_range[i]
        for j in range(n_fold):
            round_test_data = train_data[j*subset_size:(j+1)*subset_size]
            round_train_data = np.vstack((train_data[:j*subset_size],
                                          train_data[(j+1)*subset_size:]))
            ...
            round_train_lab = np.concatenate((train_labels[:j*subset_size],
                                              train_labels[(j+1) *
                                              subset_size:]))
            lab_pred = Adaboost_predict(round_train_data, round_train_lab,
                                        round_test_data, n_estim=nbound)
        micro_prec[i] = precision(round_test_lab, lab_pred,
                                  avg_type='micro')
        macro_prec[i] = precision(round_test_lab, lab_pred,
                                  avg_type='macro')
